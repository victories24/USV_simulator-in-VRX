...
class EnhancedPathTrackingController(Node):
    def __init__(self):
        ...
        self.lookahead_dist = 10.0 # 初始前视距离
        ...
        self.pid_wz = PIDController(kP=0.6, kI=0.02, kD=0.4)  
    ...
    def los_guidance(self, lookahead_point, tangent):
        ...
        # 自适应侧滑角更新（式29）
        gamma = 0.5  # 自适应增益
        U = max(0.1, abs(self.last_cmd.linear.x))  # 速度下限
        denominator = math.sqrt(self.lookahead_dist**2 + 
                            (cross_track_error + self.lookahead_dist*self.beta_hat)**2)
        #self.beta_hat += gamma * U * self.lookahead_dist * cross_track_error / denominator * 0.05
        #self.beta_hat = self.normalize_angle(self.beta_hat)  # 归一化

        beta_delta = gamma * U * self.lookahead_dist * cross_track_error / denominator * 0.05
