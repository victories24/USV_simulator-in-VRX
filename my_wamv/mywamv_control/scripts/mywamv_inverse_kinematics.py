...
class InverseKinematics(Node):
    def __init__(self):
        ...
        self.declare_parameters(
            ...
            parameters=[
                ...
                ('v_x_gain', 300.0),         # X轴线性速度增益
                ('v_y_gain', 300.0),         # Y轴线性速度增益
                ('w_z_gain', 1000.0),         # Z轴角速度增益
                ('left_pos_x', -2.373776),   # 左桨x坐标
                ('left_pos_y', 1.027135),    # 左桨y坐标
                ('right_pos_x', -2.373776),  # 右桨x坐标
                ('right_pos_y', -1.027135),  # 右桨y坐标
                ('max_thrust', 400.0),      # 最大推力值
                ('max_angle', 3.1416),      # 舵最大转向角度(180度)
                ('max_angle_rate', 0.5236), # 舵最大转向角速度(30度/秒)
                ('max_linear', 3.0),        # 小船最大速度(m/s)
                ('max_angular', 0.5),       # 小船最大角速度(rad)
                ('debug', True)             # 调试模式开关
            ]
        )
    ...
    def inverse_kinematics_callback(self, msg):
        ...
        params = self.get_parameters([
            ...
            'max_thrust', 'max_angle', 'max_angle_rate', 'max_linear',
            'max_angular','debug'
        ])
        ...
        max_angle_rate = params[10].value
        max_linear = params[11].value
        max_angular = params[12].value
        debug = params[13].value
    ...
    def config_callback(self, params):
        ...
        for param in params:
            if param.name in ['v_x_gain', 'v_y_gain', 'w_z_gain']:
                if not (0.0 <= param.value <= 500.0):
                    result.successful = False
                    result.reason = f"{param.name}必须在0.0到500.0之间"
            elif param.name == 'max_thrust':
                if not (0.0 <= param.value <= 2000.0):
                    result.successful = False
                    result.reason = "最大推力必须在0.0到2000.0之间"
