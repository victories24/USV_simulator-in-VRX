    ...
    def run(self):
        try:
            while rclpy.ok():
                key = self.get_key()
                if key == 'w':  # 增加推力
                    self.left_thrust += 10.0
                    self.right_thrust += 10.0
                elif key == 's':  # 减少推力
                    self.left_thrust -= 10.0
                    self.right_thrust -= 10.0
                elif key == 'a':  # 左转
                ...
                # 限制thrust和pos的范围
                self.left_thrust = max(-1000.0, min(1000.0, self.left_thrust))
                self.left_pos = max(-3.14, min(3.14, self.left_pos))
                self.right_thrust = max(-1000.0, min(1000.0, self.right_thrust))
