...
- 前视距离：
  ...
  载体当前位置到前视点的水平距离，记为 $$\Delta$$ 。其大小影响跟踪的平滑性和收敛速度。增大前视距离，收敛更快，但可能产生振荡；减小前视距离，跟踪更平滑，但收敛变慢。在控制策略中，需要综合考虑多种要素实时调整前视距离。
  
- 前视点：

  路径上距离载体最近点切线前方 $$\delta$$ 处的点。
  
- 横向误差：

  载体当前位置到路径的垂直距离（即最短距离）。
